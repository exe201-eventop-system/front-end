...
interface SelectedServicesProps {
    ...
    handleRemoveService: (sessionService: { id: string, serviceId?: string }) => void;
    handleBookNow: () => void;
    getMinPrice: (service: Service) => number | undefined;
    formatCurrency: (amount: number | null | undefined) => string;
}

const SelectedServices: React.FC<SelectedServicesProps> = ({ selectedServiceObjs, removingServiceId, handleRemoveService, handleBookNow, getMinPrice, formatCurrency }) => {
    return (
                    ...
                    {selectedServiceObjs.map(({ service, sessionServiceId }) => (
                        <div key={service.id} className="flex justify-between items-center p-4 bg-white rounded-lg border border-gray-200">
                            ...
                            <div className="flex items-center gap-4">
                                <p className="font-medium text-blue-600">{getMinPrice(service) !== undefined ? formatCurrency(getMinPrice(service)) : 'Chưa có'}</p>
                                <button
                                ...
                                >
                                    {removingServiceId === service.id ? 'Đang xóa...' : <Minus className="h-4 w-4" />}
                                </button>
                            </div>
                        </div>
                    ))}
